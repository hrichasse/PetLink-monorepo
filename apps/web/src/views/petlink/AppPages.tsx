...
export function DashboardPage() {
  ...
  const services = useQuery({ queryKey: ["landing-services"], queryFn: () => marketplaceApi.services.list({ isActive: true }) });
  return <Page title={role === "OWNER" ? "Hola, tu familia peluda te espera" : "Panel de proveedor"} action={role === "OWNER" ? <Button asChild variant="hero"><Link to="/services"><Search />Buscar servicio</Link></Button> : <Button asChild variant="hero"><Link to="/my-services/new"><Plus />Nuevo servicio</Link></Button>}><div className="grid gap-4 md:grid-cols-3"><Stat icon={<Heart />} label={role === "OWNER" ? "Mascotas" : "Perfil proveedor"} value={role === "OWNER" ? String(pets.data?.length ?? "—") : "Activo"} /><Stat icon={<CalendarCheck />} label="Reservas" value={String(bookings.data?.length ?? "—")} /><Stat icon={<WalletCards />} label={role === "OWNER" ? "Servicios" : "Estado"} value={role === "OWNER" ? "Real" : "Online"} /></div><section className="mt-8"><h2 className="mb-4 text-2xl font-black">Próximas reservas</h2>{bookings.isLoading ? <SkeletonGrid /> : bookings.data?.length ? <div className="grid gap-4 md:grid-cols-2">{bookings.data.slice(0, 2).map((booking) => <BookingCard key={booking.id} booking={booking} />)}</div> : <EmptyState title="Sin reservas" description="Cuando tengas reservas activas, aparecerán aquí." />}</section><section className="mt-8"><div className="mb-4 flex items-center justify-between"><h2 className="text-2xl font-black">Servicios recomendados</h2>{services.data?.length ? <Button asChild variant="soft"><Link to="/services">Ver todos</Link></Button> : null}</div>{services.isLoading ? <SkeletonGrid /> : services.data?.length ? <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">{services.data.slice(0, 3).map((service) => <Link key={service.id} to={`/services/${service.id}`}><ServiceCard service={service} /></Link>)}</div> : <div className="rounded-card border bg-card p-6 shadow-soft"><Sparkles className="h-8 w-8 text-primary" /><h3 className="mt-4 text-xl font-extrabold">Servicios conectados</h3><p className="mt-2 text-muted-foreground">Aún no hay servicios publicados por ahora.</p><Button asChild className="mt-5" variant="soft"><Link to="/services">Ver servicios</Link></Button></div>}</section></Page>;
}
...
export function ServiceDetailPage() {
  ...
  return <Page title={service?.title ?? "Servicio"} action={<Button asChild variant="outline"><Link to="/services">Volver</Link></Button>}>{isLoading ? <SkeletonGrid /> : error || !service ? <EmptyState title="Servicio no encontrado" description={error instanceof Error ? error.message : "No pudimos cargar el servicio."} /> : <div className="grid gap-6 lg:grid-cols-[1fr_360px]"><div className="space-y-4"><ServiceCard service={service} />{service.providerName ? <Link to={`/providers/${service.providerId}`} className="block text-sm font-bold text-primary hover:underline">Ver perfil de {service.providerName} →</Link> : null}<LocationMap title="Ubicación del servicio" address={service.location} city={service.city ?? null} /></div><aside className="rounded-card border bg-card p-6 shadow-soft"><h2 className="text-2xl font-black">Reserva rápida</h2><form onSubmit={form.handleSubmit((values) => mutation.mutate(values))}><label className="mt-4 block text-sm font-bold">Fecha<input type="datetime-local" className="input-shell mt-2" {...form.register("bookingDate")} /></label><label className="mt-4 block text-sm font-bold">Mascota<select className="input-shell mt-2" {...form.register("petId")}><option value="">Selecciona</option>{pets.data?.map((pet) => <option key={pet.id} value={pet.id}>{pet.name}</option>)}</select></label><Input label="Horas" type="number" error={form.formState.errors.durationHours?.message} props={form.register("durationHours")} /><Button className="mt-5 w-full" variant="hero" disabled={mutation.isPending}>{mutation.isPending ? "Reservando…" : "Confirmar reserva"}</Button></form></aside></div>}</Page>;
}

export function BookingsPage({ provider = false }: { provider?: boolean }) {
  const queryClient = useQueryClient();
  const handleError = useApiError();
  const bookingsKey = ["bookings", provider ? "provider" : "owner"] as const;
  const { data = [], isLoading, error } = useQuery({ queryKey: bookingsKey, queryFn: () => marketplaceApi.bookings.list(provider ? "provider" : "owner") });
  // Optimistically patch a booking's status in the cached list so the card
  // reflects the change instantly; roll back on error and reconcile on settle.
  const patchBookingStatus = async (id: string, status: string): Promise<Booking[] | undefined> => {
    await queryClient.cancelQueries({ queryKey: bookingsKey });
    const previous = queryClient.getQueryData<Booking[]>(bookingsKey);
    queryClient.setQueryData<Booking[]>(bookingsKey, (old) => (old ?? []).map((b) => (b.id === id ? { ...b, status: status as Booking["status"] } : b)));
    return previous;
  };
  const rollbackBookings = (previous: Booking[] | undefined) => { if (previous) queryClient.setQueryData(bookingsKey, previous); };
  const mutation = useMutation({
    mutationFn: ({ id, status }: { id: string; status: BookingStatus | Booking["status"] }) => marketplaceApi.bookings.updateStatus(id, { status: String(status).toUpperCase() as Uppercase<BookingStatus> }),
    onMutate: async ({ id, status }) => ({ previous: await patchBookingStatus(id, String(status).toUpperCase()) }),
    onError: (error, _vars, context) => { rollbackBookings(context?.previous); handleError(error); },
    onSuccess: () => { toast.success("Estado actualizado"); },
    onSettled: () => { void queryClient.invalidateQueries({ queryKey: ["bookings"] }); }
  });
  const cancelMutation = useMutation({
    mutationFn: (id: string) => marketplaceApi.bookings.cancel(id),
    onMutate: async (id) => ({ previous: await patchBookingStatus(id, "CANCELLED") }),
    onError: (error, _id, context) => { rollbackBookings(context?.previous); handleError(error); },
    onSuccess: () => { toast.success("Reserva cancelada correctamente"); },
    onSettled: () => { void queryClient.invalidateQueries({ queryKey: ["bookings"] }); }
  });
...
export function SubscriptionsPage() {
  ...
  const [selectedPlan, setSelectedPlan] = useState<"BASIC" | "PREMIUM" | "PROVIDER_PRO">("BASIC");
  const queryEnabled = !loading && !!session;

  const plans: Array<{ code: "BASIC" | "PREMIUM" | "PROVIDER_PRO"; name: string; price: number; features: string[] }> = [
    { code: "BASIC", name: "Basic", price: 9900, features: ["Perfil activo", "Funciones base"] },
    { code: "PREMIUM", name: "Premium", price: 19900, features: ["Más visibilidad", "Beneficios premium"] },
    { code: "PROVIDER_PRO", name: "Provider Pro", price: 29900, features: ["Herramientas avanzadas", "Prioridad en resultados"] }
  ];
  ...
  const myPayments = useQuery({
    ...
    enabled: queryEnabled,
    queryFn: () => marketplaceApi.payments.my()
  });
...
export function AssistantPage() {
  ...
  return (
    <Page title="PetAsistance" action={usageBadge}>
...
function ProfileField({ label, value }: { label: string; value?: string | null }) { return <div className="rounded-card border bg-background px-4 py-3"><p className="text-xs font-bold uppercase tracking-wide text-muted-foreground">{label}</p><p className="mt-2 text-base font-bold">{value?.trim() ? value : "No definido"}</p></div>; }
